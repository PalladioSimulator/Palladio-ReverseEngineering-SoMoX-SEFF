...
/**
 * Handles creation of {@link CompilationUnit Compilation Units} from java files. This class is
 * thread safe.
 *
 * <p>
 * To enable this class to optimise its performance, it should be instantiated as infrequently as
 * possible. Usually, one instance can be used throughout the whole runtime.
 *
 *
...
public class EclipseAstFactory {
    ...
    /**
     * A provider of {@link ASTParser} instances, producing a new instance for each Thread.
     */
    private static final ThreadLocal<ASTParser> AST_PARSER = ThreadLocal
     ...
     *
     * <p>
     * Calling this method with the argument {@code path} is equivalent to calling
     ...
     * @throws IOException
     *             If reading in the file at {@code path} fails.
     */
    ...
    /**
     * Reads in the Java file at the provided {@code path} and returns the
     * {@linkplain CompilationUnit} found in the file. If {@code withBindings} is {@code true}, the
     * returned compilation unit will have resolved bindings. Bindings are useful to gain
     * information about referenced types, methods, and more, but have a significant impact on time
     * and space when creating the AST. See {@link ASTParser#setResolveBindings(boolean)} for more
     * information. If bindings are being resolved,
     * {@linkplain ASTParser#setBindingsRecovery(boolean) bindings recovery} will be enabled.
     *
     * @param path
     *            The path to a Java file.
     * @param withBindings
     *            Whether to resolve bindings while creating the AST.
     * @return The compilation unit representing the java file.
     * @throws IOException
     *             If reading the file at {@code path} fails.
     */
     ...
     * {@linkplain ICompilationUnit}. If {@code withBindings} is {@code true}, the returned
     * compilation unit will have resolved bindings. Bindings are useful to gain information about
     * referenced types, methods, and more, but have a significant impact on time and space when
     * creating the AST. See {@link ASTParser#setResolveBindings(boolean)} for more information. If
     * bindings are being resolved, {@linkplain ASTParser#setBindingsRecovery(boolean) bindings
     * recovery} will be enabled.
     *
     * @param compilationUnit
     *            A JDT compilation unit.
     * @param withBindings
     *            Whether to resolve bindings while creating the AST.
     * @return The compilation unit representing the java file.
     * @throws IOException
