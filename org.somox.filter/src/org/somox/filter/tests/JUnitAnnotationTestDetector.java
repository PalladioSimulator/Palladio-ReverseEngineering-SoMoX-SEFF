...
/**
 * Detects a files containing a java class containing appropriate Annotations of JUnit as
 * being a test file. This also applies to classes that extends such a class.
 *
 * <h2>Rationale</h2>
 *
 * <p>JUnit is a framework solely used for testing purposes. The chosen annotations
 * declare methods as either containing test logic or preparing tests. If a file contains
 * such a method, it is a test file (see also the rationale in
 * {@link NotASourceFileTestDetector}). Even if JUnit or a plugin contributing to it was
 * analysed, production code would not be annotated with these annotations.
 *
 * <p>The annotations that are being searched for are:
 *
 * <ul>
 *
 * <li><code>@Test</code>
 ...
 * </ul>
 *
 * <p>Super classes are being searched because the information of these annotations are
 * being inherited (If the parent class has a test method, the child also has it).
 *
...
public class JUnitAnnotationTestDetector implements TestDetector {
	...
	/**
	 * Saves IOExceptions that might occur during stream operations to throw them after
	 * the operation.
	 */
	...
	/**
	 * Checks whether the provided {@code sourceFile} is a test file. This method is
	 * called when recursively checking superclasses.
	 *
	...
	/**
	 * Checks whether the provided {@code compilationUnit} is a test file. This method
	 * contains the actual logic and is called by {@link #isTest(ICompilationUnit)} and
	 * {@link #isTest(Path)} once the AST was obtained.
	 *
	 * @param compilationUnit A source file’s AST.
	 * @return {@code true} if {@code sourceFile} is surely a test file.
	 */
	...
	private boolean isTest(final CompilationUnit compilationUnit) {
		...
		final boolean hasJunit = containedTypes.stream()
			.flatMap(EclipseAst::withSubDeclarations)
			...
			.anyMatch(annotation -> SEARCHED_ANNOTATIONS.contains(annotation.getAnnotationType().getQualifiedName()));
		...
		return containedTypes.stream()
			.flatMap(EclipseAst::withSubDeclarations)
			.filter(EclipseAst::isTypeDeclaration)
			.map(type -> ((AbstractTypeDeclaration) type).resolveBinding())
			.filter(binding -> binding != null)
			.map(ITypeBinding::getSuperclass)
			// java.lang.Object does surely not have such a method but will very often be
			// a superclass
			.filter(superclass -> superclass != null && superclass != objectType)
			.map(ITypeBinding::getJavaElement)
			.filter(javaElement -> javaElement != null && javaElement.getElementType() == IJavaElement.COMPILATION_UNIT)
