...
public abstract class AbstractJaMoPPStatementVisitor extends ComposedSwitch<Object> {
					...
					if (!(method instanceof ClassMethod)) {
						...
						if (classMethod.getStatements() != null) {
							...
							if (KDMHelper.getJavaNodeSourceRegion(object) != null
								...
								msg.append(". Tried to call from ").append(KDMHelper.getJavaNodeSourceRegion(object).getNamespacesAsString()).append(".");
							} else {
	...
	protected String positionToString(final Commentable position) {
		...
		if (position != null) {
			...
			// KDMHelper.getSourceFile(position).getName());
			positionString.append(KDMHelper.computeFullQualifiedName(position));
			// }
	...
	protected String positionToLineNumber(final CompilationUnit position) {
