...
public class JaMoPPStatementVisitor extends AbstractJaMoPPStatementVisitor {
	...
	// AbstractMethodInvocation functionAccess = getFunctionAccess(object);
	// Block body =
	// functionAccess.getMethod().getBody();
	// if (body != null) {
	//
	...
	// "+functionAccess.getMethod().getName();
	...
	private String blockToString(final Block blockstatement) {
		...
		if (KDMHelper.getAllAccesses(blockstatement) != null &&
				KDMHelper.getAllAccesses(blockstatement).size() >= 1
				) {
			final Commentable firstAccess = KDMHelper.getAllAccesses(blockstatement).get(0);
			if (firstAccess instanceof Commentable) {
				final Commentable access = firstAccess;

				if (GetAccessedType.getAccessedType(access) != null) {
					blockString.append(" " + KDMHelper.getName(GetAccessedType.getAccessedType(access)) + "...");
				}
			}
		}
	...
	private String statementsToString(final EList<Statement> statements) {
		// TODO Remove? The method is never used
		if (statements == null) {
			return "No blockstatement";
		}
		...
		for (final Statement statement : statements) {
			...
			if (KDMHelper.getAllAccesses(statement) != null &&
					KDMHelper.getAllAccesses(statement).size() >= 1
					) {
				final Commentable firstAccess = KDMHelper.getAllAccesses(statement).get(0);
				if (firstAccess instanceof Commentable) {
					final Commentable access = firstAccess;

					if (GetAccessedType.getAccessedType(access) != null) {
						blockString
						.append(" " + KDMHelper.getName(GetAccessedType.getAccessedType(access)) + "...");
					}
					return blockString.toString();
				}
			}
